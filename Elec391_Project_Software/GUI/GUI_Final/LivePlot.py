...
class App(Tk):
    ...
    def __init__(self):
        ...
        squareButton.grid(row=0, column=1, columnspan=1, sticky="ew")
        ...
        lineButton = ttk.Button(self,
                                   text="Line",
                                   command=self.send_line)

        # triangeButton.pack(side = BOTTOM)
        lineButton.grid(row=0, column=2, sticky="ew")
    ...
    def send_shit(self):
        ...
        if self.command == "shape":
            ...
            length = len(arduino_data_prep(self.commands[self.command], self.data))
        elif self.command == "speed":
            transmissions.append(arduino_data_prep(self.commands[self.command], [int(self.data[0]*10) - 1]))
            length = len(arduino_data_prep(self.commands[self.command], self.data))
        elif self.command == "custom":
            laser_angles = [l-X_LIM_LEFT for l in self.ld.cartesian_setpoints["x"]]
            mirror_angles = [l-Y_LIM_LEFT for l in self.ld.cartesian_setpoints["y"]]
